...
export async function structureResumeText(rawText: string, pdfLinks: string[] = []): Promise<any> {
...
export async function improveBio(bio: string, targetRole: string): Promise<string> {
  const apiKey = getApiKey();
  if (!apiKey) {
...
export async function enhanceProjectDescription(title: string, desc: string, tech: string[]): Promise<string> {
  const apiKey = getApiKey();
  if (!apiKey) {
    return desc;
  }
...
function enhancedFallbackParser(rawText: string, pdfLinks: string[] = []): any {
  ...
  return {
    ...
    projects: parseProjects(
      ...
      pdfLinks.filter(link => /github\.com/i.test(link) && !link.toLowerCase().includes(extractGithubUser(githubUrl).toLowerCase())),
      pdfLinks.filter(link => !/github\.com/i.test(link) && !/linkedin\.com/i.test(link) && link !== portfolioUrl)
...
function getFirstSection(sectionMap: Record<string, string>, keys: string[]): string {
...
function findResumeName(lines: string[], email: string): string {
  ...
  if (nameLine) return nameLine.replace(/[|,]/g, ' ').replace(/\s+/g, ' ').trim();
  if (emailPrefix) return emailPrefix.replace(/\b\w/g, (char) => char.toUpperCase());
  return '';
}

function findLocation(lines: string[], email: string, phone: string): string {
...
function buildFallbackBio(summaryText: string, fullName: string, skills: any[]): string {
  const summary = summaryText.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
  if (summary) return summary.slice(0, 450);
  const skillText = skills.slice(0, 5).map((skill) => skill.name).join(', ');
  if (skillText) return `${fullName || 'This candidate'} has hands-on experience with ${skillText}.`;
  return '';
}
...
function parseAchievements(text: string): any[] {
  if (!text) return [];
  return text
    .split('\n')
...
function extractYearRange(text: string): string {
...
function cleanEducationDegree(line: string): string {
  return line
...
function findNearbyInstitution(lines: string[], startIndex: number): string {
...
function extractDateRange(text: string): string {
  return extractMatch(
    text,
    /\b((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\s*(?:[-–]\s*|\s+)(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}|Present|Current|Now)|(?:19|20)\d{2}\s*[-–]\s*(?:(?:19|20)\d{2}|Present|Current|Now))\b/i
  );
}

function cleanRoleTitle(line: string, duration: string): string {
  return line
    .replace(duration, '')
    .replace(/[-–|·•]/g, ' ')
    .replace(/\s+/g, ' ')
...
function fallbackParser(rawText: string): any {
  ...
  ) || extractMatch(rawText, /^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)/) || 'Your Name';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (keywords.some(kw => lower.includes(kw))) {
      ...
      for (let j = i + 1; j < lines.length && j < i + 40; j++) {
        ...
        if (l.length > 0 && l.length < 40 && /^[A-Z\s&\/]+$/.test(l)) break;
        section.push(l);
      }
      return section.join('\n');
    }
...
function parseProjects(text: string, githubUser: string, projectGithubLinks: string[] = [], projectLiveLinks: string[] = []): any[] {
  const results: any[] = [];
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
  ...
  for (const line of lines) {
    ...
    if (looksLikeTitle) {
      ...
      const found = line.match(techPattern) || [];
      currentProj.technologies.push(...found.map((t: string) => t));
      if (line.match(/https?:\/\//)) {
        if (line.includes('github')) currentProj.githubLink = extractMatch(line, /(https?:\/\/[^\s]+)/);
        else currentProj.liveLink = extractMatch(line, /(https?:\/\/[^\s]+)/);
      } else {
        descLines.push(line.replace(/^[-•·*]\s*/, ''));
      }
    }
...
function parseSkills(text: string): any[] {
  ...
  const softMatches = [...new Set((text.match(softPattern) || []).map(s =>
